...
6. **Réconciliation rétroactive signée par tenant.** `tenant_settings.consume_at_ship_enabled` vaut `false` par défaut. Les deux boucles du sweeper — reversal historique et rattrapage d'une consommation manquée — restent inactives jusqu'au backup, rapport et apply signés du tenant. La consommation normale go-forward dans cron/webhook/run reste indépendante du flag.
...
- `supabase/operations/2026-07-22_consume_at_ship_indexes.sql` — les deux index `CONCURRENTLY`, obligatoirement exécutés standalone hors transaction/hors pic.
- `docs/plans/2026-07-22-consume-at-ship-explain.sql` — les deux EXPLAIN non-mutants obligatoires sur FLORNA avant activation du flag tenant.
...
- `src/app/api/shipments/[id]/cancel/route.ts` — déclencher le reversal (`restockShipmentStock`) à l'annulation UI avec trois tentatives ciblées indépendantes du flag tenant.
...
- [ ] **Step 1 :** après la mise à jour `status_id=2000`, appeler `restockShipmentStock(tenantId, shipmentId)` (déjà idempotent : CAS reset `stock_consumed_at`). Faire au plus trois tentatives ciblées sans dépendre de `consume_at_ship_enabled`. Un colis annulé qui avait consommé est rendu ; s'il n'avait pas consommé, no-op. Après trois échecs, retourner explicitement `stock_reconciliation_pending=true` ; le webhook reste le dernier chemin automatique avant activation du tenant.
- [ ] **Step 2 :** tests — succès immédiat, premier appel en erreur puis succès, trois échecs bornés. Commit.
...
Objectif : rattraper deux dérives sans dépendre d'un événement temps-réel — (A) shipments **consommables, mappés, `stock_consumed_at IS NULL`** → consommer ; (B) shipments **non-consommables avec `stock_consumed_at IS NOT NULL`** → rembobiner. Borné, sérialisé, coupable. Les deux cas sont opt-in par tenant via `consume_at_ship_enabled=false` par défaut et ne s'activent qu'après la procédure signée. Cela ne désactive pas les transitions go-forward normales de cron/webhook/run.
...
- [ ] **Step 1 :** ajouter `tenant_settings.consume_at_ship_enabled boolean NOT NULL DEFAULT false`. Écrire la RPC : retourner immédiatement `(0,0)` si le flag est faux ; sinon sélectionner au plus `p_limit` shipments récents à corriger et appeler `consume_shipment_stock` (cas A) ou `restock_shipment_stock` (cas B). Verrou `FOR UPDATE SKIP LOCKED` sur le lot. `SECURITY DEFINER`, `SET search_path=public`, `REVOKE … / GRANT service_role`.
- [ ] **Step 2 :** garder `00097` entièrement transactionnelle. Créer hors migration, hors transaction et hors pic deux index partiels via `supabase/operations/2026-07-22_consume_at_ship_indexes.sql` : `idx_shipments_tenant_non_consumable_consumed` pour B, et `idx_shipments_tenant_unconsumed_shipped_recent` pour A avec `shipped_at DESC NULLS LAST`. Exécuter les deux EXPLAIN FLORNA et refuser l'activation si un plan contient un `Seq Scan` sur `shipments`, un top-N heapsort, ou ne s'arrête pas au lot de 200.
...
- [ ] **Step 2 :** mettre la sync en pause, appliquer 00096 puis 00097 via le chemin transactionnel normal. Vérifier fonctions + grants et `consume_at_ship_enabled=false` pour tous les tenants.
- [ ] **Step 3 :** exécuter standalone `supabase/operations/2026-07-22_consume_at_ship_indexes.sql` hors pic, puis vérifier `pg_index.indisvalid=true` pour les deux index.
- [ ] **Step 4 :** exécuter les deux requêtes de `docs/plans/2026-07-22-consume-at-ship-explain.sql` sur FLORNA et archiver les plans. Aucun `Seq Scan` sur `shipments`, aucun top-N heapsort, lots de 200 réellement bornés.

### Task 6.2 : Déploiement code (gate actif, sweeper actif, recalibration NON lancée)

- [ ] **Step 1 :** merger le code (gate + sweeper). À partir de là, **plus aucune nouvelle sur-conso** ne se crée. Les deux boucles rétroactives du sweeper restent inactives pour tous les tenants ; seules les transitions go-forward normales fonctionnent.
...
- **I/O (incident 13/07) :** sweeper borné (200/run/tenant), les deux directions ont un index partiel aligné, deux EXPLAIN FLORNA obligatoires, sous lock tenant, `SYNC_PAUSED` coupable, refresh mat views 1×/run inchangé, `DEFAULT_CRON_MAX_PAGES=2` inchangé.
...
- **Sweeper borné et activé tenant par tenant** : RPC + deux index standalone + orchestration + branchement cron (4.1/4.2). ✅
