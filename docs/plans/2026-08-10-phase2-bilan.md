# Phase 2 — Bilan au 10 août 2026

Correction automatique des erreurs Sendcloud. Ce document confronte ce qui
était engagé à ce qui est mesuré en production, y compris là où l'objectif
...
| Indicateur | Valeur |
|---|---|
| Corrections écrites et **vérifiées par relecture** | **323** |
| Échecs définitifs | **0** |
| Tâches bloquées | **0** |
...
### Taux d'escalade : **pas encore démontré**
...
```
03/08  11,4 %      07/08   9,3 %
04/08   0,0 %      08/08  19,4 %
05/08  12,0 %      09/08   7,4 %
06/08   7,4 %      10/08  26,2 %
```

Le pic du 10/08 est expliqué et corrigé : six escalades sur onze venaient de
signalements sur des colis **déjà étiquetés**, donc dont l'adresse avait en
réalité été acceptée par le transporteur. C'était du travail manuel fabriqué de
toutes pièces, supprimé le soir même.

Trois correctifs déployés le 10/08 devraient faire baisser ce taux, mais **ils
n'ont pas encore de mesure sur une durée significative** :
...
**Ne rien ajouter dans l'immédiat.** Le moteur est stable, sans échec ni tâche
bloquée. Les prochains correctifs utiles viendront des remontées de
l'exploitation, pas d'ajouts spéculatifs.

**Mesurer trois à quatre jours** de flux normal après les correctifs du 10/08.
C'est la seule chose qui manque pour statuer sur le critère d'escalade.

**Deux points à trancher hors développement** :

- le plan d'hébergement (l'alerte de bande passante mentionne un plan Hobby,
  alors qu'un passage en Starter était prévu en mai) ;
- l'opportunité d'ouvrir un chantier séparé pour la modification des colis déjà
  créés, seul moyen de réduire encore le résidu.
